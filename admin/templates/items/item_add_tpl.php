...
<div class="content-header row align-items-center m-0">
    <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
        <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
            ...
            <li class="breadcrumb-item active"><a href="index.html?com=items&act=man<?=$url_type?>">Danh mục <?=$csetting['name']?></a></li>
        </ol>
    </nav>
...
<div class="body-content">
    <?php if($_GET['act']=='edit' && $csetting['item-seo']==true){ ?>
    <?php foreach ($config['website']['lang'] as $k => $v) { ?>
        <?=$func->messageSeoPage($item['title_'.$k],$item['description_'.$k],$k)?>
    <?php } ?> 
    <?php } ?>
    <?=$func->messagePage($_GET['message'])?>
    <form action="index.html?com=items&act=save<?=$url_type?><?=($_GET['id']) ? '&id='.$_GET['id']:''?>" method="post" name="form-action" id="form-action"  enctype="multipart/form-data" autocomplete="off" accept-charset="utf-8">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group text-right mb-0">
                                    ...
                                    <a role="button" href="index.html?com=items&act=man<?=$url_type?>" class="btn btn-fill btn-danger">Thoát</a>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <p class="text-danger">Width: <?=$setting[$_GET['table']][$_GET['type']]['item-thumb-w']?>px - Height: <?=$setting[$_GET['table']][$_GET['type']]['item-thumb-h']?>px</p>
                                </div>
            ...
            <div class="col-lg-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <div class="form-group" style="overflow: hidden">
                                        ...
                                        <select class="form-control basic-single" name="data[id_cat]" id="id_cat">
                        ...
                        <?php if($csetting['item-mota']==true) { ?>
                        <div class="row">
                            <div class="col-md-12">
                                <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                <div class="form-group lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                                    <label class="font-weight-600">Mô tả [<?=$v?>]</label>
                                    <textarea rows="4" cols="80" <?php if($csetting['item-mota-ck']==true) { ?>class="ck_editor" data-height="300"<?php }else{ ?>class="form-control"<?php } ?> name="data[desc_<?=$k?>]" id="desc_<?=$k?>"  placeholder="Nhập mô tả"><?=$item['desc_'.$k]?></textarea>
                                </div>
                                <?php } ?>
                            </div>
                        </div>
                        <?php } ?>
                        <?php if($csetting['item-noidung']==true) { ?>
                        <div class="row">
                            <div class="col-md-12">
                                <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                <div class="form-group lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                                    <label class="font-weight-600">Nội dung [<?=$v?>]</label>
                                    <textarea rows="4" cols="80" <?php if($csetting['item-noidung-ck']==true) { ?>class="ck_editor" data-height="300"<?php }else{ ?>class="form-control"<?php } ?> name="data[content_<?=$k?>]" id="content_<?=$k?>"  placeholder="Nhập nội dung"><?=$item['content_'.$k]?></textarea>
                                </div>
                                <?php } ?>
                            </div>
                        </div>
